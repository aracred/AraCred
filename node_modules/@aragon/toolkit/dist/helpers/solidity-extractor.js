...
exports.extractContractInfo = void 0;
...
const modifiesStateAndIsPublic = declaration => !declaration.match(/\b(internal|private|view|pure|constant)\b/); // Check if the type starts with any of the basic types, otherwise it is probably
...
const getSignature = declaration => {
  let [name, params] = declaration.match(/^\s*function ([^]*?)\)/m)[1].split('(');
  ...
  if (params) {
    // Has parameters
    params = params.replace(/\n/gm, '').replace(/\t/gm, '').split(',').map(param => param.split(' ').filter(s => s.length > 0)[0]).map(type => typeOrAddress(type)).map(type => expandTypeForSignature(type)).join(',');
  }
...
const extractFunctions = async sourceCode => {
  ...
  const stateModifyingFunctionDeclarations = functionDeclarations.filter(functionDeclaration => modifiesStateAndIsPublic(functionDeclaration)).map(functionDeclaration => ({
...
const extractRoles = async functionDescriptors => {
  ...
  functionDescriptors.forEach(({
    roles
  }) => roles.forEach(role => roleSet.add(role)));
  const roleIds = [...roleSet]; // Parse role ids into objects.
  ...
  return roleIds.map(id => ({
    id,
    bytes: (0, _web3Utils.keccak256)(id),
    name: '',
    params: []
  }));
...
const extractContractInfo = async sourceCode => {
...
exports.extractContractInfo = extractContractInfo;
