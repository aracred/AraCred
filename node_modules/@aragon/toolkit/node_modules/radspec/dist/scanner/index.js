"use strict";var _interopRequireDefault=require("@babel/runtime/helpers/interopRequireDefault");var _types=_interopRequireDefault(require("../types"));Object.defineProperty(exports,"__esModule",{value:!0}),exports.scan=scan,exports.Scanner=void 0;/**
...
if("0"===current&&"x"===this.peek())for(type="HEXADECIMAL",number+=this.consume();HEX.test(this.peek());)number+=this.consume();else for(;NUMBERS.test(this.peek());)number+=this.consume();this.emitToken(type,number);break}const IDENTIFIERS=/[_$a-z0-9]/i;if(IDENTIFIERS.test(current)){let identifier=current;for(;IDENTIFIERS.test(this.peek());)identifier+=this.consume();if("true"===identifier||"false"===identifier){this.emitToken("BOOLEAN",identifier);break}_types.default.isType(identifier)?this.emitToken("TYPE",identifier):this.emitToken("IDENTIFIER",identifier);break}if("'"===current||"\""===current){let string="";for(;!this.matches("'")&&!this.matches("\"");)string+=this.consume();this.emitToken("STRING",string);break}this.report("Unexpected character \"".concat(current,"\""));}}/**
   ...
   * @param {string} error
   * @return {void}
   */report(error){this.state=SCANNER_STATE.ERROR,console.error("Error (".concat(this.cursor,"): ").concat(error))}}/**
 * Scans source and returns a list of tokens.
 *
