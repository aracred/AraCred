...
}),Object.defineProperty(exports,"providers",{enumerable:!0,get:function get(){return _rpcMessenger.providers}}),Object.defineProperty(exports,"apm",{enumerable:!0,get:function get(){return _apm.default}}),Object.defineProperty(exports,"ensResolve",{enumerable:!0,get:function get(){return _ens.resolve}}),Object.defineProperty(exports,"AddressIdentityProvider",{enumerable:!0,get:function get(){return _identity.AddressIdentityProvider}}),Object.defineProperty(exports,"getRecommendedGasLimit",{enumerable:!0,get:function get(){return _transactions.getRecommendedGasLimit}}),exports.default=exports.detectProvider=void 0;var _objectWithoutProperties2=_interopRequireDefault(require("@babel/runtime/helpers/objectWithoutProperties")),_defineProperty2=_interopRequireDefault(require("@babel/runtime/helpers/defineProperty")),_rxjs=require("rxjs"),_operators=require("rxjs/operators"),_web=_interopRequireDefault(require("web3")),_web3Utils=require("web3-utils"),_dotProp=_interopRequireDefault(require("dot-prop")),_rpcMessenger=_interopRequireWildcard(require("@aragon/rpc-messenger")),handlers=_interopRequireWildcard(require("./rpc/handlers")),_apps=_interopRequireWildcard(require("./apps")),_cache=_interopRequireDefault(require("./cache")),_apm=_interopRequireWildcard(require("./core/apm")),_repo=require("./core/apm/repo"),_aragonOS=require("./core/aragonOS"),_kernel=require("./core/aragonOS/kernel"),_configuration=require("./configuration"),configurationKeys=_interopRequireWildcard(require("./configuration/keys")),_ens=_interopRequireWildcard(require("./ens")),_identity=require("./identity"),_interfaces=require("./interfaces"),_radspec=require("./radspec"),_utils=require("./utils"),_callscript=require("./utils/callscript"),_forwarding=require("./utils/forwarding"),_intents=require("./utils/intents"),_transactions=require("./utils/transactions");function ownKeys(object,enumerableOnly){var keys=Object.keys(object);if(Object.getOwnPropertySymbols){var symbols=Object.getOwnPropertySymbols(object);enumerableOnly&&(symbols=symbols.filter(function(sym){return Object.getOwnPropertyDescriptor(object,sym).enumerable})),keys.push.apply(keys,symbols)}return keys}function _objectSpread(target){for(var source,i=1;i<arguments.length;i++)source=null==arguments[i]?{}:arguments[i],i%2?ownKeys(Object(source),!0).forEach(function(key){(0,_defineProperty2.default)(target,key,source[key])}):Object.getOwnPropertyDescriptors?Object.defineProperties(target,Object.getOwnPropertyDescriptors(source)):ownKeys(Object(source)).forEach(function(key){Object.defineProperty(target,key,Object.getOwnPropertyDescriptor(source,key))});return target}const detectProvider=()=>"undefined"==typeof web3?// eslint-disable-line
"wss://rinkeby.eth.aragon.network/ws":web3.currentProvider;/**
 * An Aragon wrapper.
 *
 ...
 */exports.detectProvider=detectProvider;class Aragon{constructor(daoAddress,options={}){const defaultOptions={defaultGasPriceFn:()=>{},provider:detectProvider(),cache:{forceLocalStorage:!1},events:{subscriptionDelayTime:0}};options=Object.assign(defaultOptions,options),(0,_configuration.setConfiguration)(configurationKeys.FORCE_LOCAL_STORAGE,!!(options.cache&&options.cache.forceLocalStorage)),(0,_configuration.setConfiguration)(configurationKeys.SUBSCRIPTION_EVENT_DELAY,Number.isFinite(options.events&&options.events.subscriptionEventDelay)?options.events.subscriptionEventDelay:0),this.web3=new _web.default(options.provider),this.ens=(0,_ens.default)(options.provider,options.apm.ensRegistryAddress);// Set up APM utilities
const{ipfs:apmIpfsOptions={}}=options.apm;// Set up the kernel proxy
// Set up cache
// Set up app contexts
this.apm=(0,_apm.default)(this.web3,{fetchTimeout:apmIpfsOptions.fetchTimeout,ipfsGateway:apmIpfsOptions.gateway}),this.kernelProxy=(0,_utils.makeProxy)(daoAddress,"Kernel",this.web3),this.cache=new _cache.default(daoAddress),this.appContextPool=new _apps.default,this.defaultGasPriceFn=options.defaultGasPriceFn}/**
...
aclAddress=await this.kernelProxy.call("acl")}catch(_){throw Error("Provided daoAddress is not a DAO")}await this.cache.init(),await this.kernelProxy.updateInitializationBlock(),await this.initAccounts(options.accounts),await this.initAcl(Object.assign({aclAddress},options.acl)),await this.initIdentityProviders(),this.initApps(),this.initForwarders(),this.initAppIdentifiers(),this.initNetwork(),this.initGuiStyle(options.guiStyle),this.pathIntents=new _rxjs.Subject,this.transactions=new _rxjs.Subject,this.signatures=new _rxjs.Subject}/**
   ...
   * @return {Promise<void>}
   */async initAccounts({fetchFromWeb3,providedAccounts=[]}={}){this.accounts=new _rxjs.ReplaySubject(1);const accounts=fetchFromWeb3?providedAccounts.concat((await this.web3.eth.getAccounts())):providedAccounts;this.setAccounts(accounts)}/**
...
const appPermissions=permissions[eventData.app]||{};if(event.event==="SetPermission"){const key="".concat(eventData.role,".allowedEntities"),allowedEntitiesSet=new Set(_dotProp.default.get(appPermissions,key,[]));// Converts to and from a set to avoid duplicated entities
eventData.allowed?allowedEntitiesSet.add(eventData.entity):allowedEntitiesSet.delete(eventData.entity),_dotProp.default.set(appPermissions,key,Array.from(allowedEntitiesSet))}event.event==="ChangePermissionManager"&&_dotProp.default.set(appPermissions,"".concat(eventData.role,".manager"),eventData.manager),permissions[eventData.app]=appPermissions}return[permissions,event]},[(0,_utils.makeAddressMapProxy)(cachedPermissions||{})]),// Cache if we're finished syncing up to cache block height
(0,_operators.map)(([permissions,event])=>(event.event===ACL_CACHE_KEY&&this.cache.set(ACL_CACHE_KEY,// Make copy for cache
...
let isForwarderOverride={};return app.isForwarder&&appInfo&&Array.isArray(appInfo.abi)&&!appInfo.abi.some(({type,name})=>"function"===type&&"isForwarder"===name)&&(isForwarderOverride={isForwarder:!1}),_objectSpread({},appInfo,{},app,{},isForwarderOverride)}))));this.apps=appsWithInfo$.pipe((0,_operators.publishReplay)(1)),this.apps.connect();/*******************************
     *                             *
     *            REPOS            *
     *                             *
...
(0,_operators.map)(apps=>Object.values(apps.filter(({appId})=>!(0,_aragonOS.isAragonOsInternalApp)(appId)).reduce((installedRepos,{appId,codeAddress,updated})=>(installedRepos[appId]={appId,updated,contractAddress:codeAddress},installedRepos),{}))),// Filter list of installed repos into:
...
(0,_operators.concatMap)(async([newRepoAppIds,updatedRepoAppIds])=>{const newRepos=(await Promise.all(newRepoAppIds.map(async appId=>{let repoProxy;try{const repoAddress=await this.ens.resolve(appId);repoProxy=(0,_repo.makeRepoProxy)(repoAddress,this.web3),await repoProxy.updateInitializationBlock()}catch(err){console.error("Could not find repo for ".concat(appId),err)}return{appId,repoProxy}}))).// Filter out repos we couldn't create proxies for (they were likely due to publishing
...
(0,_operators.mergeMap)(([newRepos,updatedRepoAppIds])=>{// Create a new observable to project each new update as its own update emission.
const update$=(0,_rxjs.of)(...updatedRepoAppIds).pipe((0,_operators.map)(appId=>({appId}))),newRepo$=(0,_rxjs.of)(...newRepos),repoAddress$=newRepo$.pipe((0,_operators.map)(({appId,repoProxy})=>({appId,repoAddress:repoProxy.address}))),version$=newRepo$.pipe(// `mergeMap()` to "flatten" the async transformation. This async function returns an
...
(0,_operators.mergeMap)(async({appId,repoProxy})=>{const initialVersions=[// Immediately query state from the repo contract, to avoid having to wait until all
...
// repo to update next.
(0,_operators.scan)(({repos},repoUpdate)=>{const{appId:updatedAppId}=repoUpdate,update=(0,_objectWithoutProperties2.default)(repoUpdate,["appId"]),updatedRepoInfo=_objectSpread({},repos[updatedAppId],{},update);return{repos:_objectSpread({},repos,{[updatedAppId]:updatedRepoInfo}),updatedRepoAppId:updatedAppId}},{repos:{},updatedRepoAppId:null}),// Stop if we don't have enough information yet to continue
(0,_operators.filter)(({repos,updatedRepoAppId})=>!!updatedRepoAppId&&Array.isArray(repos[updatedRepoAppId].versions)),// Grab the full information of the updated repo using its latest values.
// With this, we've taken the basic stream of updates for events and mapped them onto their
// full repo objects.
(0,_operators.concatMap)(async({repos,updatedRepoAppId:appId})=>{const{repoAddress,versions}=repos[appId],installedRepoInfo=installedRepoCache.get(appId),baseRepoInfo={appId,repoAddress,versions},fetchVersionInfo=version=>applicationInfoCache.request("".concat(appId,".").concat(version.contractAddress)).catch(()=>({})).then(content=>({content,version:version.version})),latestVersion=versions[versions.length-1],currentVersion=Array.from(versions)// Apply reverse to find the latest version with the currently installed contract address
...
return _objectSpread({},baseRepoInfo,{currentVersion:null,latestVersion:await fetchVersionInfo(latestVersion)});const currentVersionInfoRequest=fetchVersionInfo(currentVersion),latestVersionInfoRequest=(0,_utils.addressesEqual)(currentVersion.contractAddress,latestVersion.contractAddress)?currentVersionInfoRequest:fetchVersionInfo(latestVersion);return _objectSpread({},baseRepoInfo,{currentVersion:await currentVersionInfoRequest,latestVersion:await latestVersionInfoRequest})}));this.installedRepos=repo$.pipe(// Finally, we reduce the merged updates from individual repos into one final, expanding array
...
(0,_operators.throttleTime)(500,_rxjs.asyncScheduler,{leading:!1,trailing:!0}),(0,_operators.publishReplay)(1)),this.installedRepos.connect()}/**
   ...
   *
   * @return {void}
   */initAppIdentifiers(){this.appIdentifiers=new _rxjs.BehaviorSubject({}).pipe((0,_operators.scan)((identifiers,{address,identifier})=>Object.assign(identifiers,{[address]:identifier})),(0,_operators.publishReplay)(1)),this.appIdentifiers.connect()}/**
...
await Promise.all(identityProviders.map(({provider})=>"function"==typeof provider.init?provider.init():Promise.resolve())),this.identityProviderRegistrar=new Map(identityProviders.map(({name,provider})=>[name,provider])),this.identityIntents=new _rxjs.Subject}/**
   ...
   */modifyAddressIdentity(address,metadata){const provider=this.identityProviderRegistrar.get("local");return provider&&"function"==typeof provider.modify?provider.modify(address,metadata):Promise.reject(new Error("Provider (".concat("local",") not installed")))}/**
...
return provider&&"function"==typeof provider.resolve?provider.resolve(address):Promise.reject(new Error("Provider (".concat("local",") not installed")))}/**
...
return provider&&"function"==typeof provider.search?provider.search(searchTerm):Promise.reject(new Error("Provider (".concat("local",") not installed")))}/**
...
return this.identityProviderRegistrar.has("local")?new Promise((resolve,reject)=>{this.identityIntents.next({address,providerName:"local",resolve,reject(err){reject(err||new Error("The identity modification was not completed"))}})}):Promise.reject(new Error("Provider (".concat("local",") not installed")))}/**
...
return{shutdown,shutdownAndClearCache:async()=>(shutdown(),Object.keys((await this.cache.getAll())).reduce((promise,cacheKey)=>promise.then(()=>cacheKey.startsWith(proxyAddress)?this.cache.remove(cacheKey):Promise.resolve()),Promise.resolve()))}}}/**
   ...
   */performTransactionPath(transactionPath,{external}={}){return new Promise((resolve,reject)=>{this.transactions.next({resolve,external:!!external,transaction:transactionPath[0],path:transactionPath,reject(err){reject(err||new Error("The transaction was not signed"))}})})}/**
   ...
   *   single transaction.
   */async getExternalTransactionPath(destination,methodJsonDescription,params){let path;const installedApp=await this.getApp(destination);if(installedApp)// Destination is an installed app; need to go through normal transaction pathing
path=this.getTransactionPath(destination,methodJsonDescription.name,params);else{// Destination is not an installed app on this org, just create a direct transaction
// with the first account
const account=(await this.getAccounts())[0];try{const tx=await(0,_transactions.createDirectTransaction)(account,destination,methodJsonDescription,params,this.web3);path=this.describeTransactionPath([tx])}catch(_){}}return path||[]}/**
   ...
   */async getTransactionPathForIntentBasket(intentBasket,{checkMode="all"}={}){// Get transaction paths for entire basket
...
const sender=(await this.getAccounts())[0],directTransactions=await Promise.all(intentBasket.map(async([destination,methodSignature,params])=>(0,_transactions.createDirectTransactionForApp)(sender,(await this.getApp(destination)),methodSignature,params,this.web3)));// TODO: don't assume it's the first account
...
.map(({to})=>to).reduce((path,nextForwarder)=>{const lastStep=path[0],encodedLastStep=(0,_callscript.encodeCallScript)(Array.isArray(lastStep)?lastStep:[lastStep]);return[createForwarderTransaction(nextForwarder,encodedLastStep),...path]},// Start the recursive calls script encoding with the direct transactions for the
// intent basket
...
return{path:[],transactions:[]}}/**
   ...
   */async getPermissionManager(appAddress,roleHash){const permissions=await this.permissions.pipe((0,_operators.first)()).toPromise(),appPermissions=permissions[appAddress];return _dotProp.default.get(appPermissions,"".concat(roleHash,".manager"))}/**
   * Calculates transaction path for performing a method on the ACL
   *
   * @param {string} method
   * @param {Array<*>} params
   * @return {Promise<Array<Object>>} An array of Ethereum transactions that describe each step in the path
   */async getACLTransactionPath(method,params){const aclAddr=this.aclProxy.address,acl=await this.getApp(aclAddr),functionArtifact=acl.functions.find(({sig})=>sig.split("(")[0]===method);if(!functionArtifact)throw new Error("Method ".concat(method," not found on ACL artifact"));if(functionArtifact.roles&&0!==functionArtifact.roles.length)// createPermission can be done with regular transaction pathing (it has a regular ACL role)
...
const methodABI=acl.abi.find(item=>item.name===method&&"function"===item.type);if(!methodABI)throw new Error("Method ".concat(method," not found on ACL ABI"));const inputNames=methodABI.inputs.map(input=>input.name),appIndex=inputNames.indexOf("_app"),roleIndex=inputNames.indexOf("_role");if(-1===appIndex||-1===roleIndex)throw new Error("Method ".concat(method," doesn't take _app and _role as input. Permission manager cannot be found."));const manager=await this.getPermissionManager(params[appIndex],params[roleIndex]);return this.getTransactionPath(aclAddr,method,params,manager)}}/**
...
if(!(0,_callscript.isCallScript)(script))throw new Error("Script could not be decoded: ".concat(script));const path=(0,_callscript.decodeCallScript)(script);return path.map(segment=>{const{data}=segment;if((0,_forwarding.isValidForwardCall)(data)){const forwardedEvmScript=(0,_forwarding.parseForwardCall)(data);try{segment.children=this.decodeTransactionPath(forwardedEvmScript)}catch(err){}}return segment})}/**
...
const app=await this.getApp(destination);if(!app)throw new Error("Transaction path destination (".concat(destination,") is not an installed app"));const methods=app.functions;if(!methods)throw new Error("No functions specified in artifact for ".concat(destination));// Find the relevant method information
// Is the given method a full signature, e.g. 'foo(arg1,arg2,...)'
const fullMethodSignature=!!methodSignature&&methodSignature.includes("(")&&methodSignature.includes(")"),method=methods.find(method=>fullMethodSignature?method.sig===methodSignature// If the full signature isn't given, just select the first overload declared
:method.sig.split("(")[0]===methodSignature);if(!method)throw new Error("No method named ".concat(methodSignature," on ").concat(destination));const finalForwarderProvided=(0,_web3Utils.isAddress)(finalForwarder),directTransaction=await(0,_transactions.createDirectTransactionForApp)(sender,app,method.sig,params,this.web3);// We can already assume the user is able to directly invoke the action if:
...
const permissions=await this.permissions.pipe((0,_operators.first)()).toPromise(),destinationPermissions=permissions[destination],roleSig=app.roles.find(role=>role.id===method.roles[0]).bytes,allowedEntities=_dotProp.default.get(destinationPermissions,"".concat(roleSig,".allowedEntities"),[]);// No one has access, so of course we don't as well
